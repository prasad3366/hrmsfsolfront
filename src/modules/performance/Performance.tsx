...
const categoryVariant: Record<GoalCategory, 'default' | 'blue' | 'warning'> = { OKR: 'blue', KPI: 'default', Development: 'warning' };
const statusVariant: Record<GoalStatus, 'success' | 'warning'> = { 'On track': 'success', 'At risk': 'warning', Completed: 'success' };
...
const RatingStars = ({ rating, tone }: { rating: number; tone: 'amber' | 'ocean' }) => (
  <span className={`inline-flex items-center gap-0.5 ${tone === 'amber' ? 'text-[#b08a3e]' : 'text-[#1e627d]'}`} aria-label={`${rating} out of 5 stars`}>
    {[1, 2, 3, 4, 5].map((star) => <Star key={star} size={14} fill={star <= rating ? 'currentColor' : 'none'} strokeWidth={1.8} />)}
  </span>
);

const Modal = ({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#022337]/45 p-4 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-label={title}>
    <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-white/70 bg-[#fffefa] shadow-[0_24px_80px_rgba(2,35,55,0.24)]">
      <div className="flex items-center justify-between border-b border-[#dce6e8] bg-[#f6faf9]/70 px-5 py-4">
        <h2 className="text-lg font-bold text-[#073b5c]">{title}</h2>
        <button type="button" onClick={onClose} aria-label="Close dialog" className="rounded-lg p-2 text-[#78909a] transition hover:bg-[#edf3f5] hover:text-[#12354a]"><X size={18} /></button>
      </div>
      {children}
    </div>
  </div>
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-[#b08a3e]"><Award size={17} /> People performance</div>
          <h1 className="text-2xl font-bold tracking-tight text-[#073b5c] sm:text-3xl">Performance Management</h1>
          <p className="mt-1 text-sm text-slate-500">Align goals, review progress, and support meaningful growth.</p>
        </div>
        <Button variant="gold" className="gap-2" onClick={() => activeTab === 'goals' ? openGoal() : openReview()}><Plus size={17} /> {activeTab === 'goals' ? 'Add goal' : 'New review'}</Button>
      </div>

      <div className="flex gap-6 border-b border-[#dce6e8]" role="tablist">
        <button type="button" role="tab" aria-selected={activeTab === 'goals'} onClick={() => setActiveTab('goals')} className={`border-b-2 px-1 pb-3 text-sm font-semibold transition ${activeTab === 'goals' ? 'border-[#b08a3e] text-[#073b5c]' : 'border-transparent text-[#78909a] hover:text-[#12354a]'}`}><Target size={16} className="mr-2 inline" />Goals &amp; OKRs</button>
        <button type="button" role="tab" aria-selected={activeTab === 'reviews'} onClick={() => setActiveTab('reviews')} className={`border-b-2 px-1 pb-3 text-sm font-semibold transition ${activeTab === 'reviews' ? 'border-[#b08a3e] text-[#073b5c]' : 'border-transparent text-[#78909a] hover:text-[#12354a]'}`}><TrendingUp size={16} className="mr-2 inline" />Appraisal Reviews</button>
      </div>
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
          {goals.map((goal) => (
            <Card key={goal.id} hoverEffect className="flex flex-col rounded-xl">
              <CardHeader className="flex-row items-start justify-between gap-3"><div><Badge variant={categoryVariant[goal.category]}>{goal.category}</Badge><h2 className="mt-3 text-base font-bold text-[#12354a]">{goal.title}</h2></div><div className="flex gap-1"><button type="button" onClick={() => openGoal(goal)} aria-label={`Edit ${goal.title}`} className="rounded-lg p-2 text-[#78909a] hover:bg-[#edf3f5] hover:text-[#0d526b]"><Edit3 size={16} /></button><button type="button" onClick={() => deleteGoal(goal.id)} aria-label={`Delete ${goal.title}`} className="rounded-lg p-2 text-[#78909a] hover:bg-[#fff1ef] hover:text-[#c85d51]"><Trash2 size={16} /></button></div></CardHeader>
              <CardContent className="flex flex-1 flex-col pt-2"><p className="min-h-[48px] text-sm leading-6 text-[#617984]">{goal.description}</p><div className="mt-5"><div className="mb-2 flex justify-between text-xs font-semibold"><span className="text-[#617984]">Progress</span><span className="text-[#12354a]">{goal.progress}%</span></div><div className="h-2 overflow-hidden rounded-full bg-[#e6eeee]"><div className="h-full rounded-full bg-[#0d526b] transition-all duration-700 ease-out" style={{ width: `${goal.progress}%` }} /></div></div><div className="mt-5 flex items-center justify-between border-t border-[#edf3f5] pt-4"><span className="text-xs text-[#617984]">Due {formatDate(goal.dueDate)}</span><Badge variant={statusVariant[goal.status]}>{goal.status}</Badge></div></CardContent>
            </Card>
          ))}
        </div>
      ) : (
          <Card className="rounded-xl"><CardHeader className="border-b border-[#edf3f5]"><CardTitle>Review cycles</CardTitle><p className="text-sm text-[#617984]">Track calibration and feedback across the organization.</p></CardHeader><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>Employee name</TableHead><TableHead>Review cycle</TableHead><TableHead>Status</TableHead><TableHead>Self-rating</TableHead><TableHead>Manager rating</TableHead><TableHead>Feedback</TableHead><TableHead>Action</TableHead></TableRow></TableHeader><tbody>{reviews.map((review) => <TableRow key={review.id}><TableCell className="font-semibold text-[#12354a]">{review.employeeName}</TableCell><TableCell className="whitespace-nowrap">{review.reviewCycle}</TableCell><TableCell><span className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#486271]">{review.status === 'Completed' ? <CheckCircle2 size={15} className="text-[#19704b]" /> : review.status === 'In progress' ? <Clock size={15} className="text-[#8b641b]" /> : <AlertCircle size={15} className="text-[#78909a]" />}{review.status}</span></TableCell><TableCell><RatingStars rating={review.selfRating} tone="amber" /></TableCell><TableCell><RatingStars rating={review.managerRating} tone="ocean" /></TableCell><TableCell className="max-w-xs truncate text-[#617984]" title={review.feedback}>{review.feedback}</TableCell><TableCell><Button size="sm" variant="outline" className="gap-1.5" onClick={() => openReview(review)}><Edit3 size={14} /> Review</Button></TableCell></TableRow>)}</tbody></Table></CardContent></Card>
      )}
