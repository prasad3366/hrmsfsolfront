...
const inputClass = 'mt-2 w-full rounded-xl border border-[#cbd9dc] bg-white/80 px-3 py-2 text-sm text-[#12354a] outline-none shadow-[inset_0_1px_2px_rgba(7,59,92,0.03)] focus:border-[#b08a3e] focus:ring-2 focus:ring-[#b08a3e]/20';
...
const Toggle = ({ checked, disabled, label, onChange }: { checked: boolean; disabled?: boolean; label: string; onChange: () => void }) => <button type="button" role="switch" aria-checked={checked} aria-label={label} disabled={disabled} onClick={onChange} className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition ${checked ? 'bg-[#b08a3e]' : 'bg-[#d5e1e3]'} disabled:opacity-50`}><span className={`h-4 w-4 rounded-full bg-white shadow transition ${checked ? 'translate-x-6' : 'translate-x-1'}`} /></button>;
...
const Panel = ({ title, description, children }: { title: string; description: string; children: React.ReactNode }) => <Card className="overflow-hidden"><CardHeader className="border-b border-[#e4ecec] bg-[#f6faf9]/70"><CardTitle className="text-lg">{title}</CardTitle><p className="mt-1 text-sm font-normal text-[#617984]">{description}</p></CardHeader><CardContent className="p-5 md:p-6">{children}</CardContent></Card>;
